...
import torch.nn.functional as F  # If not already
import numpy as np
import torch
...
def extract_attn_map(visual_model, image_tensor, target_block_idx=3):
    attentions = []
    ...
    def attention_hook_fn(module, input, output):
        ...
        # Level 3: qkv.out_features=1920, C=640 ? head_dim=640, num_heads=3
        head_dim = module.qkv.out_features // 3 // C  # Should be ~1-3 heads
        
        qkv = module.qkv(x).reshape(B, N, 3, C).permute(2, 0, 1, 3)  # (3, B, N, C)
        q, k, v = qkv.unbind(0)  # Each: (B, N, C)
        
        # Compute FULL attention matrix: (B, N, N)
        attn = (q @ k.transpose(-2, -1)) * (C ** -0.5)  # Raw scores
        attn = attn.softmax(dim=-1)  # Proper attention weights
        
        attentions.append(attn.detach())  # Shape: [1, N, N] where N=784 (28x28)
    
    target_attention = visual_model.levels[3].blocks[target_block_idx].mixer
    handle = target_attention.register_forward_hook(attention_hook_fn)
    
    try:
        with torch.no_grad():
            _ = visual_model(image_tensor)
        attn_map = attentions[0] if attentions else None
        print(f"? Full attention map shape: {attn_map.shape if attn_map is not None else 'None'}")
        return attn_map
    finally:
        handle.remove()
...
def evaluate(model, data, epoch, args, tb_writer=None):
    ...
    #print(zero_shot_metrics) # just for the reference 
