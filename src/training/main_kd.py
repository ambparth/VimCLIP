...
def main(args):
    ...
    print(args.t_embed_dim, args.s_embed_dim) # just for the reference. 
    ...
    checkpoint = torch.load(args.t_model_checkpoint, map_location='cpu',weights_only=False) # this line is changed since torch version is more than 2.6
    sd = checkpoint
    # the checkpoint can be different here, 
...
##### 
## In order to print the name of the layers and the corresponding dimensions of it. ##

    #for layer_name, param in t_model.named_parameters():
    #  print(f"Layer: {layer_name}, Size: {tuple(param.size())}")

    #print("\nState dict info:")
    #for key, value in sd.items():
    #  print(f"SD Key: {key}, Size: {tuple(value.size()) if hasattr(value, 'size') else type(value)}")
...
#####    
    ...
    print(args.world_size) # test print statement. 
    loss = KDClipLoss(
        args=args,
